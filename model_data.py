...
samples = d['SHS_ID']
months = 18
# training on first 18 months out of 24
train_data = d[d['YEARMONTH'] <= 201702].reset_index(drop=True)
train_x = train_data[input_vars].as_matrix().reshape(-1,months,len(input_vars))
train_y = train_data['NRx'].as_matrix().reshape(-1,months,1)

# test on last 18 months out of 24
test_data = d[d['YEARMONTH'] >= 201603].reset_index(drop=True)
test_x = test_data[input_vars].as_matrix().reshape(-1,months,len(input_vars))
test_y = test_data['NRx'].as_matrix().reshape(-1,months,1)
